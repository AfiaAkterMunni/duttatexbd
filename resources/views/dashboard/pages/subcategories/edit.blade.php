          ...
          <div
            ...
            <form action="{{route('subcategory.update', ['id'=>$subcategory->id])}}" method="POST">
                    ...
                    <div class="pt-3">
                        ...
                        <div id="selectedImage">
                            <div class="items-center p-1 w-32 mt-5 bg-white rounded-lg shadow-xs dark:bg-gray-800" id="selectedImage">
                                <div>
                                    <img src="{{ asset('uploads/galleries/'. $subcategory->gallery->image) }}" alt="">
                                </div>
                                <div>
                                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400 text-center">{{ $subcategory->gallery->name }}</p>
                                </div>
                                <input type="hidden" name="gallery_id" value="{{ $subcategory->gallery->id }}">
                            </div>
                        </div>
                    </div>
                </label>
                ...
                <select name="category" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                    <option value="" disabled @if(is_null($subcategory->category)) selected @endif>-- Select a Category --</option>
                    @foreach ($categories as $category)
                        <option @if($subcategory->category && $category->id === $subcategory->category->id) selected @endif value="{{ $category->id }}">
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <div class="mt-8 mb-4 border-t pt-4">
                    ...
                    <label class="block text-md mb-6">
                        ...
                        <select
                            ...
                            <option value="index, all" @if($subcategory->meta_robots == 'index, all') selected @endif>Index, All</option>
                            <option value="index, follow" @if($subcategory->meta_robots == 'index, follow') selected @endif>Index, follow</option>
                            <option value="index, nofollow" @if($subcategory->meta_robots == 'index, nofollow') selected @endif>Index, no-follow</option>
                            <option value="noindex, follow" @if($subcategory->meta_robots == 'noindex, follow') selected @endif>No-index, follow</option>
                            <option value="noindex, nofollow" @if($subcategory->meta_robots == 'noindex, nofollow') selected @endif>No-index, no-follow</option>
                            <option value="noodp, noydir" @if($subcategory->meta_robots == 'noodp, noydir') selected @endif>Noodp, Noydir</option>
                        </select>
                        @error('meta_robots')
                            <p class="text-red-500 mt-4">{{$message}}</p>
                        @enderror
                    ...
                    <label class="block text-md mb-6">
                        ...
                        <input
                            class="block w-full mt-2 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                            placeholder="Enter SEO Title" name="seo_title" value="{{ $subcategory->seo_title }}"/>
                        @error('seo_title')
                            <p class="text-red-500 mt-4">{{$message}}</p>
                        @enderror
                    </label>
                    <label class="block text-md mb-6">
                        ...
                        <input
                            class="block w-full mt-2 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                            placeholder="Enter H1 Text" name="h1_text" value="{{ $subcategory->h1_text }}"/>
                        @error('h1_text')
                            <p class="text-red-500 mt-4">{{$message}}</p>
                        @enderror
                    </label>
                    <label class="block text-md mb-6">
                        ...
                        <textarea
                            class="block w-full mt-2 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                            placeholder="Enter Meta Description" name="meta_description">{{ $subcategory->meta_description }}</textarea>
                    ...
                    <label class="block text-md mb-6">
                        ...
                        <textarea
                            class="block w-full mt-2 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                            placeholder="Enter Meta Keywords" name="meta_keywords">{{ $subcategory->meta_keywords }}</textarea>
                        @error('meta_keywords')
                            <p class="text-red-500 mt-4">{{$message}}</p>
                        @enderror
                    </label>
